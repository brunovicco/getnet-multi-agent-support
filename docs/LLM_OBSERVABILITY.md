...
`src/getnet_support/adapters/observability.py`. Install it with
...
Langfuse through `src/getnet_support/adapters/tracing.py`. Structured application logging itself
is always configured through `src/getnet_support/entrypoints/logging.py` and is not part of this
